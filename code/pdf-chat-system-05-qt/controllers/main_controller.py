...
class MainController:
    ...
    def _connect_ui_events(self) -> None:
        self.window.pdf_upload_component.upload_requested.connect(self.handle_upload_pdf)
    ...
    def on_document_changed(self, document: DocumentInfo | None) -> None:
        self.window.pdf_upload_component.set_filename(
            document.filename if document else None
        )
        self.window.pdf_info_component.set_document(document)
        self.window.chat_input_component.set_has_document(document is not None)

        if document is None:
            self.window.pdf_summary_component.clear_summary()
        else:
            self.window.pdf_summary_component.set_summary(
